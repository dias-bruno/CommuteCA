...
```{r reading-tables}
HT_mode <- read.csv(paste0(measures_folder, "HT_mode_release.csv"))
HT <- read.csv(paste0(measures_folder, "HT_general_release.csv"))
SA_mode <- read.csv(paste0(measures_folder, "SA_mode_release.csv"))
SA <- read.csv(paste0(measures_folder, "SA_general_release.csv"), fileEncoding = "UTF-8-BOM")
land_use_mode <- read.csv(paste0(measures_folder, "land_use_release.csv"))
```
...
mode_metrics_da <- 
  expand.grid(PRCDDA = unique(dissemination_areas$PRCDDA), PwMode_label = unique(HT_mode$PwMode_label)) %>%
  full_join(dissemination_areas, by = "PRCDDA") %>%
  mutate(PRCDDA = as.numeric(PRCDDA)) %>%
  full_join(HT_mode, by = c("PRCDDA", "PwMode_label")) %>%
  full_join(SA_mode, by = c("PRCDDA", "PwMode_label")) %>% 
  full_join(land_use_mode[,c("PRCDDA","PwMode_label","labour_force")], by = c("PRCDDA", "PwMode_label"))
...
land_use_general <- land_use_mode %>% 
  ...
  summarise(labour_force = sum(labour_force, na.rm = TRUE), 
            jobs = max(jobs_rounded, na.rm = TRUE))
...
total_pop_by_mode_DA_2 <- tm_shape(mode_metrics_da_sf) + 
  tm_polygons("labour_force",
              ...
              title = " ",
              border.col = NULL) +  
  tm_facets(by = "PwMode_label", free.scales = TRUE) +       
  tm_scale_bar(position = c("right", "bottom")) +
   tm_compass(position = c("left", "top"), size = 1.0) + 
   tm_shape(census_divisions) +
   tm_borders("black", lwd=0.5)
