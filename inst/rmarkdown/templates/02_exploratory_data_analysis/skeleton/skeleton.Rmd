...
data contains 30,000 rows and over 30 columns, for respondents residing the Greater Toronto Metropolitan and Hamilton, in Ontario. As in the original census
...
| CfInc_AT | Total income of census family (sum of the total incomes of all members of that family), after tax. |
...
different folder that does not follow the pattern of the `CommuteCA`
package.
...
census_filtered <-  census %>% 
  dplyr::select("Frame_ID", "PRCDDA","PR","CMA",
         "PCD","CompW1","LBR_FORC",
         "CfInc_AT","PWDA","PWPR","PWCD","PWCMA",
         "PWDUR","PWDist","PwMode")
  

summary(census_filtered)
```
...
census_filtered  <- census_filtered  %>% 
          ...
          mutate(PwMode_label = case_when(
                  ...
                  PwMode > 0 & PwMode <= 2 ~ "Car-motorized",
...
```{r}
summary(census_filtered$PwMode_label)
```
...
```{r}
census_filtered <- census_filtered %>%
  mutate(across(c(PWDA, PWPR, 
                  PWCD, PWCMA), ~ ifelse(. == -3, "Not applicable", .)))
```
...
```{r}
census_filtered <- census_filtered %>%
  mutate(Pr_label = case_when(PR == 10 ~ "Newfoundland and Labrador",
...
```{r}
example_list <- c(20, 30, 32, 34, 41, 41, 45, 46, 48, 51, 
                  53, 54, 54, 56, 57, 58, 58, 59, 60, 61, 
                  64, 65, 65, 69, 71, 74, 77, 79, 88, 94)
```
...
`LBR_FORC` variable (considering the test data), representing around 60%
...
```{r}
census_city %>% 
  dplyr::summarise(Median = Hmisc::wtd.quantile(CfInc_AT, 
                                                weights = CompW1, probs = 0.5))
```
...
```{r}
median(census_city$CfInc_AT) # WRONG! You need to consider the weights! 
```
...
```{r}
#mean(census$CfInc_AT) # WRONG! You need to consider the weights! 

census_city %>% 
  dplyr::summarise(Mean = Hmisc::wtd.mean(CfInc_AT, weights = CompW1))
```
...
```{r}
census_city %>% 
  dplyr::summarise(Mean = sum(CfInc_AT * CompW1)/sum(CompW1))
```
...
`CfInc_AT`. In fact, we applied the quantile function to obtain the median
...
```{r}
census_city %>% 
  dplyr::summarise(Second_Q = Hmisc::wtd.quantile(CfInc_AT, 
                                                  weights = CompW1, probs = 0.25),
                   Median = Hmisc::wtd.quantile(CfInc_AT, 
                                                weights = CompW1, probs = 0.5), 
                   Third_Q = Hmisc::wtd.quantile(CfInc_AT, 
                                                 weights = CompW1, probs = 0.75))
```
...
census_city %>% 
  dplyr::summarise(Mean = Hmisc::wtd.mean(CfInc_AT, weights = CompW1),
                   Variance = Hmisc::wtd.var(CfInc_AT, weights = CompW1),
                   SD = sqrt(Variance))
```
...
maximum of `CfInc_AT` (total income of census family):

```{r}
CfInc_AT_statistics <- census_city %>% 
  dplyr::summarise(Mean = Hmisc::wtd.mean(CfInc_AT, weights = CompW1),
                   Median = Hmisc::wtd.quantile(CfInc_AT, 
                                                weights = CompW1, probs = 0.5),
                   SD = sqrt(Hmisc::wtd.var(CfInc_AT, 
                                            weights = CompW1)), 
                   Max = max(CfInc_AT),
                   Min = min(CfInc_AT))
...
in the test data set for city of Toronto was R\$ $230,001$. Just how
...
```{r}
CfInc_AT_statistics$Max - CfInc_AT_statistics$Mean
```

That is, approximately R\$ $166,396.3$. But the typical deviation from the
mean in the sample was about R\$ $63,604.72$! Now, calculating:

```{r}
(CfInc_AT_statistics$Max - CfInc_AT_statistics$Mean)/CfInc_AT_statistics$SD
```
...
```{r}
CfInc_AT_statistics$Min
```

That is, around \$ $998$. Again, the typical deviation from the mean in
the sample was R\$ $63,604.72$! So, calculating:

```{r}
(CfInc_AT_statistics$Min - CfInc_AT_statistics$Mean)/CfInc_AT_statistics$SD
```
...
census_city %>%
  ...
  summarise(weighted_n = sum(CompW1, na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(names_from = PwMode_label, 
              values_from = weighted_n, values_fill = 0)
```
...
weights::wtd.cor(x = census_city$PWDUR, 
                 y = census_city$PWDist, weight = census_city$CompW1)
```

The code above shows that the variables `PWDUR` and `PWDist` have a very
high correlation (above 0.9), which makes sense, as travel time tends
...
What is the central tendency of `CfInc_AT` (total income of census family )
taking into account only the numbers shown? Is the `PWDist` more or less
spread than the `CfInc_AT` (distance (straight-line) from home to work)?
...
implemented as `geom_hist()`. Using the variable `CfInc_AT` (total income
of census family), we have:

```{r}
ggplot(data = census_city,
       aes(x = CfInc_AT, weight = CompW1)) +
  geom_histogram()
```
...
the `CfInc_AT`), because the "count" on the y-axis is a computed statistic.
...
ggplot(data = census_city,
 ...
 scale_x_continuous(breaks=seq(0,max(census_city$CfInc_AT),50000))
```
...
ggplot(data = census_city,
       aes(x = CfInc_AT)) +
  geom_density(bins = 20) +
  geom_vline(xintercept = CfInc_AT_statistics$Mean,
             color = "blue",
             size = 1) +
  geom_vline(xintercept = CfInc_AT_statistics$Median,
             color = "green",
             size = 1)
```
...
statistics like the mean of a quantitative variable (e.g., `CfInc_AT`) are
...
census_city |>
  ...
  dplyr::summarize(mean_CfInc_AT = Hmisc::wtd.mean(CfInc_AT, weights = CompW1)) %>%  
  ggplot() +
...
ggplot(data = census_city_expanded) +
  geom_density_ridges(aes(x = CfInc_AT, y = PwMode_label)) 
```
...
recreates the boxplot of `PwMode` and `CfInc_AT`, and further maps
`LBR_FORC` to color:
