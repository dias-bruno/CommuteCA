...
Creating a Data Frame with workers who live and work in the city under
study:
...
Create a new Data Frame with all possible combinations of DA code and
...
After creating the traditional spatial filter based on a contiguity
matrix, we will now explore the potential of building a spatial filter
based on a time matrix. First, we will create a binary matrix, where the
values will be 0 if the commute duration is higher than the median
duration, and 1 otherwise.

Next, we will explore the possibility of building a spatial filter using
the impedance values themselves. To do so, we will normalize the
impedance values so they range from 0 to 1.
