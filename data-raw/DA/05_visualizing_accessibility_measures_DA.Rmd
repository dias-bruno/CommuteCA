...
land_use_mode <- read.csv(paste0(measures_folder, "land_use_original.csv"))
```
...
mode_metrics_da <- 
  ...
  full_join(land_use_mode[,c("PRCDDA","PwMode_label","labour_force")], by = c("PRCDDA" = "PRCDDA", "PwMode" = "PwMode_label"))
...
```{r}
# General land use
land_use_general <- land_use_mode %>% 
  group_by(PRCDDA) %>% 
  summarise(labour_force = sum(labour_force, na.rm = TRUE), 
            jobs = max(jobs, na.rm = TRUE))
...
```{r directory-export}
diretorio_export_figures <- paste0(here(),"/data-raw/output/",code,"/DA/accessibility-figures/") # Update the address to export the data, if necessary
...
Plotting the labour force by area and job
opportunities by area in each dissemination_areas:

```{r}
labour_force_km_plot <- HT_metrics_da %>% 
  mutate(area_km2 = as.numeric(st_area(.)) / 1e6,
         labour_force_density = labour_force / area_km2) %>%
  tm_shape() + 
  tm_polygons("labour_force_density",
              style = "cont",
              palette = "Reds",
              title = " ",
              border.col = NULL) + 
  ...
  tm_layout(legend.outside = FALSE,
            legend.position = c("left", "bottom"),
            panel.labels = expression("Labour force per km"^2))

jobs_km_plot <-  SA_metrics_da %>% 
  mutate(area_km2 = as.numeric(st_area(.)) / 1e6,
         job_density = jobs / area_km2) %>%
  tm_shape() + 
  ...
  tm_layout(legend.outside = FALSE,
            legend.position = c("left", "bottom"),
            panel.labels = expression("Jobs per km"^2))

lf_jobs_km_DA <- tmap_arrange(labour_force_km_plot, jobs_km_plot, ncol = 2)

tmap_save(lf_jobs_km_DA, 
          paste0(diretorio_export_figures,"lf_jobs_km_DA.jpg"), 
          width = 15, 
          height = 15, 
          units = "cm", 
          dpi = 300)
...
```{r}
knitr::include_graphics(paste0(diretorio_export_figures,"/lf_jobs_km_DA.jpg"))
```

```{r}
lf_jobs_km_DA_2 <- tmap_arrange(labour_force_plot, labour_force_km_plot, 
                              jobs_plot, jobs_km_plot, ncol = 2)

tmap_save(lf_jobs_km_DA_2, 
          paste0(diretorio_export_figures,"lf_jobs_km_DA_2.jpg"), 
          width = 15, 
          height = 18, 
          units = "cm", 
          dpi = 300)
...
```{r}
knitr::include_graphics(paste0(diretorio_export_figures,"/lf_jobs_km_DA_2.jpg"))
```
...
jobs_per_labour_force <- tm_shape(HT_metrics_da) + 
  ...
  tm_layout(legend.outside = FALSE,
            legend.position = c("left", "bottom"),
            panel.labels = "Jobs per labour fource")
...
SA_per_labour_force <- tm_shape(SA_metrics_da) + 
  ...
  tm_layout(legend.outside = FALSE,
            legend.position = c("left", "bottom"),
            panel.labels = "Spatial availability per labour force")
...
```{r}
total_pop <- sum(land_use_mode$labour_force, na.rm = TRUE)
total_SA <- sum(mode_metrics_da$SA_im, na.rm = TRUE)
