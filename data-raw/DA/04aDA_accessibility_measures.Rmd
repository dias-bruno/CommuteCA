...
```{r bike-function}
mode <- 'Bike'
bike_function <- test_distributions_weighted(x = tld_empirical[tld_empirical$mode == mode, ]$PWDUR, 
...
bike_function_df <- data.frame("PwMode_label"=  as.factor(mode), 
                               ...
                               "est_1" = ifelse(name_function$distname == "unif", 0, name_function$estimate[1]),
                               "est_2" = ifelse(name_function$distname == "unif",
                                               name_function$estimate[1], ifelse(name_function$distname == "exp", 0, name_function$estimate[2])))
```
...
```{r car-function}
mode <- 'Car-motorized'
car_function <- test_distributions_weighted(x = tld_empirical[tld_empirical$mode == mode, ]$PWDUR, 
                                    weights = round(tld_empirical[tld_empirical$mode == mode, ]$CompW1))
car_function
...
```{r transit-function}
mode <- 'Public transit'
transit_function <- test_distributions_weighted(x = tld_empirical[tld_empirical$mode == mode, ]$PWDUR, 
                                    weights = round(tld_empirical[tld_empirical$mode == mode, ]$CompW1))
transit_function
...
```{r walk-function}
mode <- 'Walk'
walk_function <- test_distributions_weighted(x = tld_empirical[tld_empirical$mode == mode, ]$PWDUR, 
                                    weights = round(tld_empirical[tld_empirical$mode == mode, ]$CompW1))
walk_function
...
function based on the empirical length distributions. For that, we use the `generate_impedance` from `CommuteCA`:
...
for(mode in unique(ttm$PwMode_label)){
  ...
  mode_function <- all_functions %>%
    filter(PwMode_label == mode)
  
   ttm_f_mode$f <- CommuteCA::generate_impedance(mode_function,
                                travel_cost = ttm_f_mode$travel_time)
   
   ttm_f_mode$distribution <-  mode_function$distribution
   ttm_f_mode$est_1 <-  mode_function$est_1
   ttm_f_mode$est_2 <-  mode_function$est_2
...
land_use_support <- land_use_support %>%
  ...
  dplyr::select(PRCDDA, PwMode_label, labour_force, jobs, jobs_respondents)
...
accessibility_table <- ttm_f %>% 
  ...
  left_join(land_use[,c("PRCDDA","PwMode_label", "jobs")], by = c("PWDA" = "PRCDDA", "PwMode_label"))
...
keep <- c("census", "census_backup", "land_use", "accessibility_table", "code", "land_use_support")
all_objects <- ls()
...
```{r}
SA_pp_mode_active <- SA_mode_active %>% 
  mutate(SA_pp_im = ifelse(labour_force == 0, 0, SA_im/labour_force))
```
...
# Spatial Availability 
SA_pp_general_support <- SA_pp %>% 
  left_join(weighted_DA, by = c('PRCDDA' = 'PRCDDA')) %>% 
  dplyr::select(PRCDDA, SA_i, SA_pp_i, Weighted_pop_DA)

SA_pp_mode_support <- SA_pp_mode %>% 
  left_join(vetting_accessibility, by = c('PRCDDA' = 'PRCDDA','PwMode_label' = 'PwMode_label')) %>%
  dplyr::select(PRCDDA, PwMode_label, SA_im, SA_pp_im, Weighted_pop_DA)

SA_pp_mode_active_support <- SA_pp_mode_active %>% 
  left_join(vetting_accessibility_3mode, by = c('PRCDDA' = 'PRCDDA','PwMode_label' = 'PwMode_label')) %>%
  dplyr::select(PRCDDA, PwMode_label, SA_im, SA_pp_im, Weighted_pop_DA)
```
...
SA_pp_general_support <- SA_pp_general_support  %>%
  filter(Weighted_pop_DA >= 40)

HT_mode_support <- HT_mode_support %>%
  filter(Weighted_pop_DA >= 40)

SA_pp_mode_support <- SA_pp_mode_support %>%
  filter(Weighted_pop_DA >= 40)

SA_pp_mode_active_support <- SA_pp_mode_active_support %>% 
    filter(Weighted_pop_DA >= 40)
```
...
```{r directory-export}
diretorio_export <- paste0(here(),"/data-raw/output/PCD",code,"/DA/accessibility-measures/") # Update the address to export the data, if necessary
...
# HT tables
write.csv(HT, paste0(diretorio_export, "/original/HT_general_original.csv"), row.names=FALSE)
write.csv(HT_mode, paste0(diretorio_export, "/original/HT_mode_original.csv"), row.names=FALSE)

# SA tables 
write.csv(SA_pp, paste0(diretorio_export, "/original/SA_general_original.csv"), row.names=FALSE)
write.csv(SA_pp_mode, paste0(diretorio_export, "/original/SA_pp_mode_original.csv"), row.names=FALSE)
write.csv(SA_pp_mode_active, paste0(diretorio_export, "/original/SA_pp_mode_active.csv"), row.names=FALSE)
```

## Export of Hansen tables:

```{r export-HT_mode}
write.csv(HT_mode_support[,c("PRCDDA","PwMode_label","HT_im")], paste0(diretorio_export, "/release/HT_mode_release.csv"), row.names=FALSE)
write.csv(HT_mode_support, paste0(diretorio_export, "/support/HT_mode_support.csv"), row.names=FALSE)
```

```{r export-HT}
write.csv(HT_general_support[,c("PRCDDA","HT_i")], paste0(diretorio_export, "/release/HT_general_release.csv"), row.names=FALSE)
write.csv(HT_general_support, paste0(diretorio_export, "/release/HT_general_support.csv"), row.names=FALSE)
```
...
saveWorkbook(ht_excel, paste0(diretorio_export, "/release/HT_release.xlsx"), overwrite = TRUE)
...
saveWorkbook(ht_excel, paste0(diretorio_export, "/support/HT_support.xlsx"), overwrite = TRUE)
```

## Export of Spatial Availability tables:

```{r export-SA-pp}
write.csv(SA_pp_general_support[,c("PRCDDA","SA_i","SA_pp_i")], paste0(diretorio_export, "/release/SA_general_release.csv"), row.names=FALSE)
write.csv(SA_pp_general_support, paste0(diretorio_export, "/support/SA_general_support.csv"), row.names=FALSE)
```

```{r export-SA-mode}
write.csv(SA_pp_mode_support[,c("PRCDDA","PwMode_label","SA_im","SA_pp_im")], paste0(diretorio_export, "/release/SA_mode_release.csv"), row.names=FALSE)
write.csv(SA_pp_mode_support, paste0(diretorio_export, "/support/SA_mode_support.csv"), row.names=FALSE)
```

```{r export-SA_pp_mode_active}
write.csv(SA_pp_mode_active_support[,c("PRCDDA","PwMode_label","SA_im", "SA_pp_im")], paste0(diretorio_export, "/release/SA_mode_active_release.csv"), row.names=FALSE)
write.csv(SA_pp_mode_active_support, paste0(diretorio_export, "/support/SA_mode_active_support.csv"), row.names=FALSE)
```
...
writeData(sa_excel, "general", SA_pp_general_support[,c("PRCDDA","SA_i", "SA_pp_i")])
writeData(sa_excel, "mode",  SA_pp_mode_support[,c("PRCDDA","PwMode_label","SA_im","SA_pp_im")])
writeData(sa_excel, "3mode",  SA_pp_mode_active_support[,c("PRCDDA","PwMode_label","SA_im","SA_pp_im")])

saveWorkbook(sa_excel, paste0(diretorio_export, "/release/SA_release.xlsx"), overwrite = TRUE)
...
writeData(sa_excel, "general", SA_pp_general_support)
writeData(sa_excel, "mode",  SA_pp_mode_support)
writeData(sa_excel, "3mode",  SA_pp_mode_active_support)

saveWorkbook(sa_excel, paste0(diretorio_export, "/support/SA_support.xlsx"), overwrite = TRUE)
```
...
```{r export-SA}
# land use tables
write.csv(land_use_support, paste0(diretorio_export, "/original/land_use_original.csv"), row.names=FALSE)
```

Accessibilities:

```{r export-SA}
accessibilities_mode <- SA_pp_mode_support %>% 
  left_join(HT_mode_support %>%
              dplyr::select(PRCDDA, PwMode_label, HT_im),
            by = c("PRCDDA", "PwMode_label"))

accessibilities_general <- SA_pp_general_support %>% 
...
write.csv(accessibilities_mode %>% 
          ...
          paste0(diretorio_export, "/release/accessibilities_mode_release.csv"), row.names=FALSE)

write.csv(accessibilities_mode,
          paste0(diretorio_export, "/support/accessibilities_mode_support.csv"), row.names=FALSE)

write.csv(accessibilities_general %>% 
            dplyr::select(-Weighted_pop_DA),
          paste0(diretorio_export, "/release/accessibilities_general_release.csv"), row.names=FALSE)

write.csv(accessibilities_general, 
          paste0(diretorio_export, "/support/accessibilities_general_support.csv"), row.names=FALSE)
```
