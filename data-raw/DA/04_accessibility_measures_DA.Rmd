---
title: "Calculating the accessibility and spatial availability of jobs (Dissemination area level)"
...
1.  First, we estimate the travel time for each transportation mode using the [{r5r}](https://cloud.r-project.org/web/packages/fitdistrplus/index.html) package [@Pereira2021], considering all possible combinations of dissemination areas of the selected study area. 
...
census_filtered <- census %>%
   filter(PCD == code | PWCD == code) %>% # Select those who live or work in the census division
   filter(LBR_FORC %in% c(1,2)) # Select those who are in the labour force 
...
census_filtered <- census_filtered  %>% 
                  ...
                  PwMode_label = factor(PwMode_label, 
                                        levels = c("Bike", "Walk", 
                                                   "Car-motorized", "Public transit", "Other or non-commuter")))
...
census_employed <- census_employed %>%
  mutate(PWDUR = if_else(PWDUR == 0, 0.01, PWDUR)) # Besides it is possible a travel-time of 0 minutes, we need to substitute this value because the functions used bellow to define the best impedance function does not accept zero as a valid value. Because of this, we'll change all zero values to 0.1 minute travel-time.
...
tld_empirical <- rbind(df_tt_bike,
  ...
  mutate(mode = factor(mode,
                       levels = c("Bike", "Walk", "Car-motorized", "Public transit")),
...
weights_df <- census_employed %>%
  group_by(PRCDDA,
           PWDA,
           PwMode_label) %>%
  dplyr::summarize(weights = sum(CompW1),
            .groups = "drop")

ttm <- ttm %>% 
  left_join(weights_df, by = c("from_id" = "PRCDDA", 
                               "to_id" = "PWDA", 
...
for(pcd in unique(ttm$PCD)){

  # PCD data set
  ttm_pcd <- ttm[ttm$PCD == pcd, ]
  
  cat("Obtaining the impedance function for the PCD: ", code, "\n")

  for(mode in (unique(ttm_pcd$PwMode_label))){
    
    cat("Transportation mode: ", mode, "\n")

    ttm_pcd_mode <- ttm_pcd[ttm_pcd$PwMode_label == mode, ]

    if(length(ttm_pcd_mode$travel_time) == 1){

      ttm_pcd_mode$dist <- 'No dist.'

      ttm_pcd_mode$f <- 1

      ttm_f <- rbind(ttm_f, ttm_pcd_mode)


    } else {

      best_mode_function <- test_distributions(x = ttm_pcd_mode$travel_time, 
                                                        weights = ttm_pcd_mode$weights)
      if(best_mode_function$distname == "lnorm"){
             fname <-  best_mode_function$distname
             estimate_1 <- best_mode_function$estimate[1]
             estimate_2 <- best_mode_function$estimate[2]
            }
            
            # Unif impedance 
            else if(best_mode_function$distname == "unif") {
             fname <-  best_mode_function$distname
             estimate_1 <- 0
             estimate_2 <- best_mode_function$estimate[2]
             
            # Exponential impedance 
            } else if(best_mode_function$distname == "exp") {
             fname <-  best_mode_function$distname
             estimate_1 <- best_mode_function$estimate
             estimate_2 <- 0 
             
                # Gamma impedance 
            } else if (best_mode_function$distname == "gamma"){  
             fname <-  best_mode_function$distname
             estimate_1 <- best_mode_function$estimate[1]
             estimate_2 <- best_mode_function$estimate[2]
             
            # Norm impedance 
            } else if (best_mode_function$distname == "norm"){
             fname <-  best_mode_function$distname
             estimate_1 <- best_mode_function$estimate[1]
             estimate_2 <- best_mode_function$estimate[2]
            }
      
      ttm_pcd_mode$dist <- fname
      ttm_pcd_mode$est_1 <- estimate_1
      ttm_pcd_mode$est_2 <- estimate_2

      ttm_pcd_mode$f <- generate_impedance(mode_function = best_mode_function,
                                           travel_cost = ttm_pcd_mode$travel_time)
...
non_commuters <- census_filtered %>%
  filter(PwMode == -3 | PwMode == 10) %>% # Considering the category 'Other' as non-commuter
  group_by(PRCDDA) %>% 
...
grid_exp <- expand.grid(PRCDDA = union(non_commuters$PRCDDA, commuters$PRCDDA), 
                        PwMode_label = unique(commuters$PwMode_label))

non_commuters_exp <- grid_exp %>% 
  left_join(non_commuters, by = "PRCDDA")

commuters_exp <- grid_exp %>% 
  left_join(commuters, by = c("PRCDDA","PwMode_label"))

labour_force_support <- commuters_exp %>% 
  left_join(non_commuters_exp, by = c("PRCDDA", "PwMode_label"))

labour_force_support <- labour_force_support %>%
  mutate(all_non_commuters = if_else(is.na(all_non_commuters), 0, all_non_commuters),
...
labour_force_support <- labour_force_support %>%
  left_join(general_perc_mode %>% dplyr::select(PwMode_label, perc_pcd), by = "PwMode_label") 
...
labour_force_support <- labour_force_support %>%
  mutate(commuters_mode = coalesce(commuters_mode, 0),
         commuters_mode_n = coalesce(commuters_mode_n, 0),
         all_commuters  = coalesce(all_commuters, 0),
         all_commuters_n  = coalesce(all_commuters_n, 0),
         all_non_commuters_n  = coalesce(all_non_commuters_n, 0),
         labour_force = (all_non_commuters * perc_final) + commuters_mode, 
         lf_respondents = all_non_commuters_n + all_commuters_n) %>%
...
grid_exp_jobs <- expand.grid(PWDA = unique(jobs$PWDA), PwMode_label = unique(commuters$PwMode_label))

jobs <- left_join(jobs, grid_exp_jobs)  %>%
  mutate(PWDA = as.character(PWDA)) %>%
  filter(startsWith(PWDA, as.character(code)))
```
...
```{r create-land-use}
land_use_support <- data.frame(expand.grid(PRCDDA = union(labour_force$PRCDDA, jobs$PWDA), 
                                   PwMode_label = unique(commuters$PwMode_label)))

land_use_support <- land_use_support %>% 
  full_join(labour_force_support, by = c("PRCDDA","PwMode_label")) %>%
  full_join(jobs, by = c("PRCDDA" = "PWDA","PwMode_label"))
```
...
land_use_support <- land_use_support %>%
  ...
  dplyr::select(PRCDDA, PwMode_label, labour_force, lf_respondents, jobs, jobs_respondents)
...
accessibility_table <- ttm_f %>% 
  dplyr::rename('PRCDDA' = from_id,
         'PWDA' = to_id) %>% 
  left_join(land_use[,c("PRCDDA","PwMode_label","labour_force")], by = c("PRCDDA" = "PRCDDA", "PwMode_label" = "PwMode_label")) %>%
  left_join(land_use[,c("PRCDDA","PwMode_label","jobs")], by = c("PWDA" = "PRCDDA","PwMode_label" = "PwMode_label"))
...
keep <- c("census", "census_backup", "commuters", "test_distributions_weighted", "land_use", "accessibility_table", "ttm_f" , "ttm_folder", "code", "land_use_support")
all_objects <- ls()
...
- *Statistics must be based on at least 5 respondents, whose sum of weights is at least 10*.
...
vetting_accessibility <- weighted_mode %>%
  ...
  dplyr::select(PRCDDA, PwMode_label, Weighted_pop_DA, Respondents_DA, Respondents_mode, Weighted_pop_mode)
```
...
HT_general_support <- HT %>% 
  ...
  dplyr::select(PRCDDA, HT_i, Weighted_pop_DA, Respondents_DA)

HT_mode_support <- HT_mode %>%
  left_join(vetting_accessibility, by = c('PRCDDA' = 'PRCDDA','PwMode_label' = 'PwMode_label')) %>%
  dplyr::select(PRCDDA, PwMode_label, HT_im, Weighted_pop_DA, Weighted_pop_mode, Respondents_mode)

# Spatial Availability 
SA_general_support <- SA %>% 
  left_join(weighted_DA, by = c('PRCDDA' = 'PRCDDA')) %>% 
  dplyr::select(PRCDDA, SA_i, Weighted_pop_DA, Respondents_DA)

SA_mode_support <- SA_mode %>% 
  left_join(vetting_accessibility, by = c('PRCDDA' = 'PRCDDA','PwMode_label' = 'PwMode_label')) %>%
  dplyr::select(PRCDDA, PwMode_label, SA_im, Weighted_pop_DA, Weighted_pop_mode, Respondents_mode)
```
...
```{r}
land_use_mode_support <- land_use_support %>% 
  left_join(weighted_DA, by = c('PRCDDA' = 'PRCDDA')) %>% 
  filter(Weighted_pop_DA >= 40,
         labour_force >= 10,
         lf_respondents >= 5,
  ...
  dplyr::select("PRCDDA", "PwMode_label", "labour_force", "lf_respondents", 
                "jobs_rounded", "jobs", "jobs_respondents", "Weighted_pop_DA")
```
...
```{r directory-export}
diretorio_export <- paste0(here(),"/data-raw/output/PCD3520/DA/accessibility-measures/") # Update the address to export the data, if necessary
...
write.csv(HT_mode[,c("PRCDDA","PwMode_label","HT_im")], paste0(diretorio_export, "HT_mode_original.csv"), row.names=FALSE)

# SA tables
write.csv(SA[,c("PRCDDA","SA_i")], paste0(diretorio_export, "SA_general_original.csv"), row.names=FALSE)
write.csv(SA_mode[,c("PRCDDA","PwMode_label","SA_im")], paste0(diretorio_export, "SA_mode_original.csv"), row.names=FALSE)
```

Export of Hansen tables:

```{r export-HT_mode}
write.csv(HT_mode_support[,c("PRCDDA","PwMode_label","HT_im")], paste0(diretorio_export, "HT_mode_release.csv"), row.names=FALSE)
write.csv(HT_mode_support, paste0(diretorio_export, "HT_mode_support.csv"), row.names=FALSE)
```

```{r export-HT}
write.csv(HT_general_support[,c("PRCDDA","HT_i")], paste0(diretorio_export, "HT_general_release.csv"), row.names=FALSE)
write.csv(HT_general_support, paste0(diretorio_export, "HT_general_support.csv"), row.names=FALSE)
```


Saving HT in excel files: 
...
writeComment(ht_excel, "general", col = 1, row = 1, 
             comment = createComment("Identification of the Dissemination area", author = "Author"))
writeComment(ht_excel, "general", col =2 , row = 1, 
             comment = createComment("Score of job accessibility that the residents from this DA have, using the Hansen measure.", author = "Author"))

writeComment(ht_excel, "mode", col = 1, row = 1, 
             comment = createComment("Identification of the Dissemination area", author = "Author"))
writeComment(ht_excel, "mode", col =2 , row = 1, 
             comment = createComment("Transportation mode used to commute to work.", author = "Author"))
writeComment(ht_excel, "mode", col =3 , row = 1, 
             comment = createComment("Score of job accessibility that the residents from this DA have using this transportation mode mentioned in PwMode_label, using the Hansen measure.", author = "Author"))

cols_with_values_general <- which(sapply(HT_general_support[,c("PRCDDA","HT_i")], function(x) any(!is.na(x))))
addStyle(ht_excel, sheet = c("general"),
         style = createStyle(fgFill = "yellow"),
         rows = 1:(nrow(HT_general_support)+1),
         cols = cols_with_values_general,
         gridExpand = TRUE)

cols_with_values_mode <- which(sapply(HT_mode_support[,c("PRCDDA","PwMode_label","HT_im")], function(x) any(!is.na(x))))
addStyle(ht_excel, sheet = c("mode"),
         style = createStyle(fgFill = "yellow"),
         rows = 1:(nrow(HT_mode_support)+1),
         cols = cols_with_values_mode,
         gridExpand = TRUE)

saveWorkbook(ht_excel, paste0(diretorio_export, "HT_release.xlsx"), overwrite = TRUE)

# HT Release
ht_excel_support <- createWorkbook()

addWorksheet(ht_excel_support, "general")
addWorksheet(ht_excel_support, "mode")

writeData(ht_excel_support, "general", HT_general_support)
writeData(ht_excel_support, "mode", HT_mode_support)

writeComment(ht_excel_support, "general", col = 1, row = 1, 
             comment = createComment("Identification of the Dissemination area", author = "Author"))
writeComment(ht_excel_support, "general", col =2 , row = 1, 
             comment = createComment("Score of job accessibility that the residents from this DA have, using the Hansen measure.", author = "Author"))
writeComment(ht_excel_support, "general", col = 3, row = 1, 
             comment = createComment("Total population (weighted) in this DA.", author = "Author"))
writeComment(ht_excel_support, "general", col = 4, row = 1, 
             comment = createComment("Number of respondents (unweighted count) in this DA.", author = "Author"))

writeComment(ht_excel_support, "mode", col = 1, row = 1, 
             comment = createComment("Identification of the Dissemination area", author = "Author"))
writeComment(ht_excel_support, "mode", col =2 , row = 1, 
             comment = createComment("Transportation mode used to commute to work.", author = "Author"))
writeComment(ht_excel_support, "mode", col =3 , row = 1, 
             comment = createComment("Score of job accessibility that the residents from this DA have using this transportation mode mentioned in PwMode_label, using the Hansen measure.", author = "Author"))
writeComment(ht_excel_support, "mode", col = 4, row = 1, 
             comment = createComment("Total population (weighted) in this DA.", author = "Author"))
writeComment(ht_excel_support, "mode", col = 5, row = 1, 
             comment = createComment("Total population (weighted) in this DA who commute using the transporation mode mentioned in PwMode_label.", author = "Author"))
writeComment(ht_excel_support, "mode", col = 6, row = 1, 
             comment = createComment("Number (unweighted count) of respondents in this DA who commute using the transporation mode mentioned in PwMode_label.", author = "Author"))

addStyle(ht_excel_support, sheet = c("general"),
         style = createStyle(fgFill = "yellow"),
         rows = 1:(nrow(HT_general_support)+1),
         cols = cols_with_values_general,
         gridExpand = TRUE)

addStyle(ht_excel_support, sheet = c("mode"),
         style = createStyle(fgFill = "yellow"),
         rows = 1:(nrow(HT_mode_support)+1),
         cols = cols_with_values_mode,
         gridExpand = TRUE)

saveWorkbook(ht_excel_support, paste0(diretorio_export, "HT_support.xlsx"), overwrite = TRUE)
```

Export of spatial availability tables:

```{r export-SA}
write.csv(SA_general_support[,c("PRCDDA","SA_i")], paste0(diretorio_export, "SA_general_release.csv"), row.names=FALSE)
write.csv(SA_general_support, paste0(diretorio_export, "SA_general_support.csv"), row.names=FALSE)
```

```{r export-SA_mode}
write.csv(SA_mode_support[,c("PRCDDA","PwMode_label","SA_im")], paste0(diretorio_export, "SA_mode_release.csv"), row.names=FALSE)
...
writeData(sa_excel, "general", SA_general_support[,c("PRCDDA","SA_i")])
writeData(sa_excel, "mode", SA_mode_support[,c("PRCDDA","PwMode_label","SA_im")])

## Commenting and highlighting the general sheet

writeComment(sa_excel, "general", col = 1, row = 1, 
             comment = createComment("Identification of the Dissemination area", author = "Author"))
writeComment(sa_excel, "general", col =2 , row = 1, 
             comment = createComment("Score of job accessibility that the residents from this DA have, using the Spatial availability measure.", author = "Author"))

writeComment(sa_excel, "mode", col = 1, row = 1, 
             comment = createComment("Identification of the Dissemination area", author = "Author"))
writeComment(sa_excel, "mode", col =2 , row = 1, 
             comment = createComment("Transportation mode used to commute to work.", author = "Author"))
writeComment(sa_excel, "mode", col =3 , row = 1, 
             comment = createComment("Score of job accessibility that the residents from this DA have using this transportation mode mentioned in PwMode_label, using the Spatial availability measure.", author = "Author"))

cols_with_values_general <- which(sapply(SA_general_support[,c("PRCDDA","SA_i")], function(x) any(!is.na(x))))
addStyle(sa_excel, sheet = c("general"),
         style = createStyle(fgFill = "yellow"),
         rows = 1:(nrow(SA_general_support)+1),
         cols = cols_with_values_general,
         gridExpand = TRUE)

cols_with_values_mode <- which(sapply(SA_mode_support[,c("PRCDDA","PwMode_label","SA_im")], function(x) any(!is.na(x))))
addStyle(sa_excel, sheet = c("mode"),
         style = createStyle(fgFill = "yellow"),
         rows = 1:(nrow(SA_mode_support)+1),
         cols = cols_with_values_mode,
         gridExpand = TRUE)

saveWorkbook(sa_excel, paste0(diretorio_export, "SA_release.xlsx"), overwrite = TRUE)

# SA Support Release
sa_excel_support <- createWorkbook()

addWorksheet(sa_excel_support, "general")
addWorksheet(sa_excel_support, "mode")

writeData(sa_excel_support, "general", SA_general_support)
writeData(sa_excel_support, "mode", SA_mode_support)

writeComment(sa_excel_support, "general", col = 1, row = 1, 
             comment = createComment("Identification of the Dissemination area", author = "Author"))
writeComment(sa_excel_support, "general", col =2 , row = 1, 
             comment = createComment("Score of job accessibility that the residents from this DA have, using the Spatial availability measure.", author = "Author"))
writeComment(sa_excel_support, "general", col = 3, row = 1, 
             comment = createComment("Total population (weighted) in this DA.", author = "Author"))
writeComment(sa_excel_support, "general", col = 4, row = 1, 
             comment = createComment("Number of respondents (unweighted count) in this DA.", author = "Author"))

writeComment(sa_excel_support, "mode", col = 1, row = 1, 
             comment = createComment("Identification of the Dissemination area", author = "Author"))
writeComment(sa_excel_support, "mode", col =2 , row = 1, 
             comment = createComment("Transportation mode used to commute to work.", author = "Author"))
writeComment(sa_excel_support, "mode", col =3 , row = 1, 
             comment = createComment("Score of job accessibility that the residents from this DA have using this transportation mode mentioned in PwMode_label, using the Spatial availability measure.", author = "Author"))
writeComment(sa_excel_support, "mode", col = 4, row = 1, 
             comment = createComment("Total population (weighted) in this DA.", author = "Author"))
writeComment(sa_excel_support, "mode", col = 5, row = 1, 
             comment = createComment("Total population (weighted) in this DA who commute using the transporation mode mentioned in PwMode_label.", author = "Author"))
writeComment(sa_excel_support, "mode", col = 6, row = 1, 
             comment = createComment("Number (unweighted count) of respondents in this DA who commute using the transporation mode mentioned in PwMode_label.", author = "Author"))

addStyle(sa_excel_support, sheet = c("general"),
         style = createStyle(fgFill = "yellow"),
         rows = 1:(nrow(SA_general_support)+1),
         cols = cols_with_values_general,
         gridExpand = TRUE)

addStyle(sa_excel_support, sheet = c("mode"),
         style = createStyle(fgFill = "yellow"),
         rows = 1:(nrow(SA_mode_support)+1),
         cols = cols_with_values_mode,
         gridExpand = TRUE)

saveWorkbook(sa_excel_support, paste0(diretorio_export, "SA_support.xlsx"), overwrite = TRUE)
```
...
```{r export-SA}
# land use tables
write.csv(land_use_support, paste0(diretorio_export, "land_use_original.csv"), row.names=FALSE)

write.csv(land_use_mode_support %>% 
          dplyr::select(PRCDDA, PwMode_label, labour_force, jobs_rounded), 
          paste0(diretorio_export, "land_use_release.csv"), row.names=FALSE)
```

Saving land use in excel format for release: 

```{r}
lu_excel <- createWorkbook()

addWorksheet(lu_excel, "mode")

writeData(lu_excel,  "mode", land_use_mode_support %>% 
            dplyr::select(PRCDDA, PwMode_label, labour_force, jobs_rounded))

## Commenting and highlighting the general sheet

writeComment(lu_excel, "mode", col = 1, row = 1, 
             comment = createComment("Identification of the Dissemination area", author = "Author"))
writeComment(lu_excel, "mode", col = 2, row = 1, 
             comment = createComment("Transportation mode used to commute to work.", author = "Author"))
writeComment(lu_excel, "mode", col = 3, row = 1, 
             comment = createComment("Estimated number of people in labour force that commute using the transportation mode described in PwMode.", author = "Author"))
writeComment(lu_excel, "mode", col = 4, row = 1, 
             comment = createComment("Number of people who mentioned working at this dissemination area (rounded).", author = "Author"))

addStyle(lu_excel, sheet = c("mode"),
         style = createStyle(fgFill = "yellow"),
         rows = 1:(nrow(land_use_mode_support)+1),
         cols = c(1:4),
         gridExpand = TRUE)

saveWorkbook(lu_excel, paste0(diretorio_export, "land_use_release.xlsx"), overwrite = TRUE)

# Land Use Support 
lu_excel_support <- createWorkbook()

addWorksheet(lu_excel_support, "mode")

writeData(lu_excel_support, "mode",  land_use_mode_support)

## Commenting and highlighting the general sheet
writeComment(lu_excel_support, "mode", col = 1, row = 1, 
             comment = createComment("Identification of the Dissemination area", author = "Author"))
writeComment(lu_excel_support, "mode", col = 2, row = 1, 
             comment = createComment("Transportation mode used to commute to work.", author = "Author"))
writeComment(lu_excel_support, "mode", col = 3, row = 1, 
             comment = createComment("Estimated number of people in labour force that commute using the transportation mode described in PwMode.", author = "Author"))
writeComment(lu_excel_support, "mode", col = 4, row = 1, 
             comment = createComment("Number of respondents used to estimate the number of people in labour force.", author = "Author"))
writeComment(lu_excel_support, "mode", col = 5, row = 1, 
             comment = createComment("Total of people (weighted) who mentioned working at this dissemination area (rounded).", author = "Author"))
writeComment(lu_excel_support, "mode", col = 6, row = 1, 
             comment = createComment("Total of people (weighted) who mentioned working at this dissemination area (not rounded).", author = "Author"))
writeComment(lu_excel_support, "mode", col = 7, row = 1, 
             comment = createComment("Number of respondent who mentioned working at this dissemination area (not rounded).", author = "Author"))
writeComment(lu_excel_support, "mode", col = 8, row = 1, 
             comment = createComment("Total population (weighted) in this dissemination area.", author = "Author"))

addStyle(lu_excel_support, sheet = c("mode"),
         style = createStyle(fgFill = "yellow"),
         rows = 1:(nrow(land_use_mode_support)+1),
         cols = c(1:8),
         gridExpand = TRUE)

saveWorkbook(lu_excel_support, paste0(diretorio_export, "land_use_support.xlsx"), overwrite = TRUE)
```

Exporting the ttm_f table (not for release):

```{r export-ttm_f}
write.csv(ttm_f, paste0(ttm_folder, "ttm_f.csv"), row.names=FALSE)
```

Legend for th exported files: 
