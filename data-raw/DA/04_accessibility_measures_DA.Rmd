...
```{r read-ttm}
ttm_file <- paste0(here(),"/data-raw/output/PCD3520/DA/travel_times/ttm.csv")
...
for(pcd in unique(ttm$PCD)){
  ...
  for(mode in (unique(ttm_pcd$PwMode))){
    ...
    if(length(ttm_pcd_mode$travel_time) == 1){
      ...
      ttm_pcd_mode$dist <- best_mode_function$distname
...
keep <- c("census", "commuters", "census_labour_force", "test_distributions_weighted", "land_use", "ttm_f","accessibility_table", "code")
all_objects <- ls()
