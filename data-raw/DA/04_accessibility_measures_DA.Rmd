...
census_filtered <- census_filtered  %>% 
                  ...
                  PwMode_label = factor(PwMode_label, 
                                        levels = c("Not applicable", "Bike", "Walk", 
                                                   "Car-motor", "Transit", "Other")))
...
df_tt_car <- census_employed %>% 
  filter(PwMode_label == 'Car-motor') %>%
  dplyr::select(PWDUR, CompW1) %>%
  mutate(mode = "Car-motor")

# Transit travel times
df_tt_transit <- census_employed %>% 
  filter(PwMode_label == 'Transit') %>%
  dplyr::select(PWDUR, CompW1) %>%
  mutate(mode = "Transit")
...
ttm <- ttm %>% 
  left_join(weights_df, by = c("from_id" = "PRCDDA", "to_id" = "PWDA", "PwMode_label" = "PwMode_label"))
