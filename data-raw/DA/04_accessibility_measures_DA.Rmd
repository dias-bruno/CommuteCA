...
for(pcd in unique(ttm$PCD)){
  ...
  for(mode in (unique(ttm_pcd$PwMode))){
    
    cat("Transportation mode: ", mode, "\n")

    ttm_pcd_mode <- ttm_pcd[ttm_pcd$PwMode == mode, ]
...
keep <- c("census", "commuters", "census_labour_force", "test_distributions_weighted", "land_use", "accessibility_table", "ttm_f" , "ttm_folder", "code")
all_objects <- ls()
