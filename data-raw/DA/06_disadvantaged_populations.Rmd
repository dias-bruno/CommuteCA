---
title: "Selecting disadvantaged population groups in terms of accessibility  (_Dissemination area_ level)"
...
| CFAMSTSIMPLE | This variable refers to whether a person was employed, unemployed or not in the labour force. |
...
```{r}
# Reading the test Census data set
files_address <- paste0(here(),"/data-raw/make-data/census_test_toronto.csv")
...
'employed.'[^7]. In addition, the labour force is made up of the
employed population and the unemployed population. This variable can
assume the values:

[^7]: Those who, during the reference period: (a) Did any work at all at
...
following possible values:[^8]

[^8]: Given that the non-binary population is small, data aggregation to
...
[^] Refers to the income situation of a person in relation to the Low-income cut-offs, after-tax (LICO-AT). Persons with income that is below this low-income line are considered to be in low income.

- -3: Concept not applicable (collective households)
- 1: Not in low income
- 2: In low income
- 3: Concept not applicable (territories or reserves) - not in low income
- 4: Concept not applicable (territories or reserves) - in low income

According to the census code book, the variable 'LoLIMA' has the
following possible values:[^]

[^] Refers to the income situation of a person in relation to the Low-income measure, after-tax (LIM-AT). Persons with income that is below this low-income line are considered to be in low income.
...
- 5: Concept not applicable (collective households) - not in low income
- 6: Concept not applicable (collective households) - in low income

We will use the variable *ACTLMFLG - Activities of daily living: Flag* as an indicative of disability, because the Census of Population does not have a variable regarding disability. The question on activities of daily living refers to difficulties a person may have doing certain activities as a result of physical, cognitive, mental, or other health-related conditions or problems. We will select all those respondents who affirm to have frequent (always) difficulties to develop regular activities. According to the census code book, the variable 'ACTLMFLG' has the
following possible values:
...
According to the census code book, the variable 'CFAMSTSIMPLE' has the following possible values:
...
According to the census code book, the variable 'IMMCAT5' has the following possible values:
...
According to the census code book, "Visible minority" refers to whether a person is a visible minority as defined by the Employment Equity Act. The Employment Equity Act defines visible minorities as "persons, other than Aboriginal peoples, who are non-Caucasian in race or non-white in colour." The visible minority population consists mainly of the following groups: South Asian, Chinese, Black, Filipino, Arab, Latin American, Southeast Asian, West Asian, Korean and Japanese. The variable 'DVisMin' has the following possible values:
...
In the next chunk, we will rename the travel modes to facilitate the readability of the data.
...
In the previous rmarkdown, we calculate accessibility measures at the DA level. Now, we will load the csv file with the general spatial availability measures (not distinct by mode):

```{r reading-accessibility-files}
# For the original accessibility_measures data set
files_address <- paste0(here(),"/data-raw/aux-files/PCD3520/DA/output/SA_general_original.csv")
...
Now, we will identify the differences in accessibility by population groups. However, it is important to remember that the census data has weights for each respondent. Because of this, we will expand the census file, repeating the respondent line based on their weight. This means that, a person with weight = 2, will appear two times in this new data frame:
...
Creating new data frames to save summary information about the populations: 
...
Starting with the youth population, we will consider as the disadvantaged group all respondents younger than 15-years. 
...
The previous chunk display the median and mean values for the disadvantaged/non-disadvantaged groups. The following chuck save this information in the summary dataframe: 
...
df <- df %>%
  ...
  select(Variable, Description, Category, Equity, Total_pop, Respondents, Median, Mean, Median_rate, Mean_rate)
...
For gender, we will consider as the disadvantaged group all respondents who identify themselves as Woman+ (the + indicates non-binary population, but due to low representation of this group had to be merged in a binary classification): 
...
For labour force, we will consider as the disadvantaged group all unemployed respondents:  
...
We will consider as the disadvantaged group all low income respondents (LICO or LIMA): 
...
As a disability indicator, we will consider as the disadvantaged group all respondents who reported frequent difficulties to develop regular activities:  
...
We will consider as the disadvantaged group all respondents who are refugees. 
...
We will consider as the disadvantaged group all respondents who are new immigrants (those who immigrated after 2016): 
...
We will consider as the disadvantaged group all respondents who are in a visible minority group. 
...
Now, we will analyze differences in accessibility within the visible minority groups: 
...
The following chunk display the median values for each disadvantaged/non-disadvantaged group: 
...
Using the ggplot library, we will visualize these differences in form of bar plots. 

Setting the output folder address: 

```{r}
output_folder <- paste0(here::here(),"/data-raw/aux-files/PCD3520/DA/output/")
``` 
...
# Saving figure
ggsave(file = paste0(output_folder,"/medians_by_pop_group.jpg"), 
...
```{r median-figure}
knitr::include_graphics(paste0(output_folder,"/medians_by_pop_group.jpg"))
```
...
# Saving figure
ggsave(file = paste0(output_folder,"/ratio_medians_by_pop_group.jpg"), 
...
```{r}
knitr::include_graphics(paste0(output_folder,"/ratio_medians_by_pop_group.jpg"))
```

## Visible minorities

Using the ggplot library, we will visualize these differences in form of bar plots. First, visualizing the medians of between visible minority groups: 
...
# Saving figure
ggsave(file = paste0(output_folder,"/medians_by_racial_group.jpg"), 
...
```{r}
knitr::include_graphics(paste0(output_folder,"/medians_by_racial_group.jpg"))
```
...
# Saving figure
ggsave(file = paste0(output_folder,"/ratio_medians_by_racial_group.jpg"), 
...
```{r}
knitr::include_graphics(paste0(output_folder,"/ratio_medians_by_racial_group.jpg"))
```
...
We are performing the analysis at the level of a City (PCD), however, to facilitate the work of the RDC analyst, we will also include these information to our table outputs: 

```{r vetting}
census_pcd_vetting <- census_pcd %>%
  group_by(PCD) %>%
  summarize(Population_PCD = sum(CompW1),
            Respondents_PCD = n()) %>% 
...
aux_pop_table <- summary_pop_table_support %>% 
  group_by(Description) %>%
  summarize(Population_Group = sum(Total_pop),
...
pvalue_table_support <- pvalue_table %>% 
  ...
  mutate(PCD = code_pcd) %>% 
  left_join(census_pcd_vetting, by = "PCD") %>% 
  dplyr::select(Variable, Description, Population_Group, Respondents_Group, P_value, Significance, PCD, Population_PCD, Respondents_PCD)
```
...
```{r directory-export}
diretorio_export <- paste0(here(),"/data-raw/aux-files/PCD3520/DA/output/")
```
...
# Release - all groups
write.csv(summary_pop_table_support[,c("Variable","Description","Category","Equity","Total_pop_rounded", "Median", "Mean", "Median_rate", "Mean_rate")], paste0(diretorio_export, "summary_pop_table_release.csv"), row.names=FALSE)

# Supporting file for RDC - all groups 
write.csv(summary_pop_table_support, paste0(diretorio_export, "summary_pop_table_support.csv"), row.names=FALSE)

# Release - visible minorities 
write.csv(summary_dvismin_table_support[,c("Variable","Description","Category","Equity","Total_pop_rounded", "Median", "Mean", "Median_rate", "Mean_rate")], paste0(diretorio_export, "summary_vmin_table_release.csv"), row.names=FALSE)

# Supporting file for RDC - visible minorities 
write.csv(summary_dvismin_table_support, paste0(diretorio_export, "summary_vmin_table_support.csv"), row.names=FALSE)
```

Export of p-values tables:
```{r export-summary-tab}
write.csv(pvalue_table_support[,c("Variable", "Description", "P_value",  "Significance")], paste0(diretorio_export, "pvalue_table_release.csv"), row.names=FALSE)

write.csv(pvalue_table_support, paste0(diretorio_export, "pvalue_table_support.csv"), row.names=FALSE)
```
