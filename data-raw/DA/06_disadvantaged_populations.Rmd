...
files_address <- "C:\\Bruno\\CommuteCA\\data-ignore-inputs\\census_test_toronto_v3.csv"
census <- read.csv(files_address, header = TRUE)
```
...
files_address <- paste0(here(),"/data-raw/output/PCD",code_pcd,"/DA/accessibility-measures/SA_general_original.csv")
...
summary_pop_table <- data.frame(Variable = character(),
                                ...
                                Total_pop = numeric(),
                                Respondents = numeric(),
                                NonZeroCases = numeric(),
                                NonZeroCases_weighted = numeric(),
                                Median = numeric(),
                                Mean = numeric(),
                                Mean_rounded = numeric(),
                                Median_rounded = numeric(),
                                Median_rate = numeric(),
                                Mean_rate = numeric(),
...
get_p <- function(nonzero_pop) {
  case_when(
    nonzero_pop < 10      ~ 0.1667,
    nonzero_pop < 20      ~ 0.1667,
    nonzero_pop < 40      ~ 0.10,
    nonzero_pop < 80      ~ 0.05,
    nonzero_pop < 160     ~ 0.025,
    nonzero_pop < 320     ~ 0.0125,
    nonzero_pop < 640     ~ 0.00625,
    nonzero_pop < 1280    ~ 0.00313,
    TRUE                  ~ 0.00156
  )
}

percent_round <- function(value, nonzero_pop) {
  p <- get_p(nonzero_pop)
  base <- pmax(1, round(abs(value) * p))  
...
census_accessibility %>% 
  ...
  dplyr::summarise(
    "Total_pop" = sum(CompW1, na.rm = TRUE), 	"Respondents" = n(), NonZeroCases = sum(Total_SA > 0),
    "NonZeroCases_weighted"  = sum(CompW1[Total_SA > 0], na.rm = TRUE),
...
```{r}
df_youth <- census_accessibility %>% 
  group_by(!!sym(variable)) %>%
  summarise(
    Total_pop              = sum(CompW1, na.rm = TRUE),
    Respondents            = n(),
    NonZeroCases           = sum(Total_SA > 0),                        # unweighted
    NonZeroCases_weighted  = sum(CompW1[Total_SA > 0], na.rm = TRUE),  # weighted
    Median                 = Hmisc::wtd.quantile(Total_SA, weights = CompW1, probs = 0.5),
    Mean                   = sum(Total_SA * CompW1, na.rm = TRUE) / sum(CompW1, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
    Mean_rounded      = percent_round(Mean, NonZeroCases_weighted),
    Median_rounded    = percent_round(Median, NonZeroCases_weighted),
  ) %>%
  rename(Category = !!sym(variable))

# Rates
medr_disad <- 
  df_youth %>% filter(Category == non_disadvantaged) %>% pull(Median_rounded) / 
  df_youth %>% filter(Category == disadvantaged) %>% pull(Median_rounded) 
...
meanr_disad <- 
  df_youth %>% filter(Category == non_disadvantaged) %>% pull(Mean_rounded) / 
  df_youth %>% filter(Category == disadvantaged) %>% pull(Mean_rounded)
...
df_youth <- df_youth %>%
 ...
 dplyr::select(Variable, Description, Category, Equity, Total_pop, Respondents, NonZeroCases, NonZeroCases_weighted, Median, Mean, Mean_rounded, Median_rounded, Median_rate, Mean_rate)


summary_pop_table <- bind_rows(summary_pop_table, df_youth) 
```
...
```{r}
df_gender <- census_accessibility %>% 
  group_by(!!sym(variable)) %>%
  summarise(
    Total_pop              = sum(CompW1, na.rm = TRUE),
    Respondents            = n(),
...
# Rates
medr_disad <- 
  df_gender %>% filter(Category == non_disadvantaged) %>% pull(Median_rounded) / 
  df_gender %>% filter(Category == disadvantaged) %>% pull(Median_rounded) 
...
meanr_disad <- 
  df_gender %>% filter(Category == non_disadvantaged) %>% pull(Mean_rounded) / 
  df_gender %>% filter(Category == disadvantaged) %>% pull(Mean_rounded)
...
summary_pop_table <- bind_rows(summary_pop_table, df_gender) 
```
...
```{r}
df_labour <- census_accessibility %>% 
  group_by(!!sym(variable)) %>%
  summarise(
    Total_pop              = sum(CompW1, na.rm = TRUE),
    Respondents            = n(),
...
# Rates
medr_disad <- 
  df_labour %>% filter(Category == non_disadvantaged) %>% pull(Median_rounded) / 
  df_labour %>% filter(Category == disadvantaged) %>% pull(Median_rounded) 
...
meanr_disad <- 
  df_labour %>% filter(Category == non_disadvantaged) %>% pull(Mean_rounded) / 
  df_labour %>% filter(Category == disadvantaged) %>% pull(Mean_rounded)
...
summary_pop_table <- bind_rows(summary_pop_table, df_labour) 
```
...
```{r}
df_income <- census_accessibility %>% 
  group_by(!!sym(variable)) %>%
  summarise(
    Total_pop              = sum(CompW1, na.rm = TRUE),
    Respondents            = n(),
...
# Rates
medr_disad <- 
  df_income %>% filter(Category == non_disadvantaged) %>% pull(Median_rounded) / 
  df_income %>% filter(Category == disadvantaged) %>% pull(Median_rounded) 
...
meanr_disad <- 
  df_income %>% filter(Category == non_disadvantaged) %>% pull(Mean_rounded) / 
  df_income %>% filter(Category == disadvantaged) %>% pull(Mean_rounded)
...
summary_pop_table <- bind_rows(summary_pop_table, df_income) 
```
...
```{r}
df_disability <- census_accessibility %>% 
  group_by(!!sym(variable)) %>%
  summarise(
    Total_pop              = sum(CompW1, na.rm = TRUE),
    Respondents            = n(),
...
# Rates
medr_disad <- 
  df_disability %>% filter(Category == non_disadvantaged) %>% pull(Median_rounded) / 
  df_disability %>% filter(Category == disadvantaged) %>% pull(Median_rounded) 
...
meanr_disad <- 
  df_disability %>% filter(Category == non_disadvantaged) %>% pull(Mean_rounded) / 
  df_disability %>% filter(Category == disadvantaged) %>% pull(Mean_rounded)

meanr_ad <- 
  df_disability %>% filter(Category == disadvantaged) %>% pull(Mean_rounded) / 
  df_disability %>% filter(Category == non_disadvantaged) %>% pull(Mean_rounded)

df_disability <- df_disability %>%
  mutate(Variable = variable,
    Description = description,
    Equity = ifelse(Category == disadvantaged, "Disadvantaged", "Non-disadvantaged"),
    Median_rate = ifelse(Category == disadvantaged, medr_ad, medr_disad),
...
summary_pop_table <- bind_rows(summary_pop_table, df_disability) 
```
...
```{r}
df_single_parent <- census_accessibility %>% 
  group_by(!!sym(variable)) %>%
  summarise(
    Total_pop              = sum(CompW1, na.rm = TRUE),
    Respondents            = n(),
...
# Rates
medr_disad <- 
  df_single_parent %>% filter(Category == non_disadvantaged) %>% pull(Median_rounded) / 
  df_single_parent %>% filter(Category == disadvantaged) %>% pull(Median_rounded) 
...
meanr_disad <- 
  df_single_parent %>% filter(Category == non_disadvantaged) %>% pull(Mean_rounded) / 
  df_single_parent %>% filter(Category == disadvantaged) %>% pull(Mean_rounded)

meanr_ad <- 
  df_single_parent %>% filter(Category == disadvantaged) %>% pull(Mean_rounded) / 
  df_single_parent %>% filter(Category == non_disadvantaged) %>% pull(Mean_rounded)

df_single_parent <- df_single_parent %>%
  mutate(Variable = variable,
    Description = description,
    Equity = ifelse(Category == disadvantaged, "Disadvantaged", "Non-disadvantaged"),
    Median_rate = ifelse(Category == disadvantaged, medr_ad, medr_disad),
...
summary_pop_table <- bind_rows(summary_pop_table, df_single_parent) 
```
...
```{r}
df_refugee <- census_accessibility %>% 
  group_by(!!sym(variable)) %>%
  summarise(
    Total_pop              = sum(CompW1, na.rm = TRUE),
    Respondents            = n(),
...
# Rates
medr_disad <- 
  df_refugee %>% filter(Category == non_disadvantaged) %>% pull(Median_rounded) / 
  df_refugee %>% filter(Category == disadvantaged) %>% pull(Median_rounded) 
...
meanr_disad <- 
  df_refugee %>% filter(Category == non_disadvantaged) %>% pull(Mean_rounded) / 
  df_refugee %>% filter(Category == disadvantaged) %>% pull(Mean_rounded)

meanr_ad <- 
  df_refugee %>% filter(Category == disadvantaged) %>% pull(Mean_rounded) / 
  df_refugee %>% filter(Category == non_disadvantaged) %>% pull(Mean_rounded)

df_refugee <- df_refugee %>%
  mutate(Variable = variable,
    Description = description,
    Equity = ifelse(Category == disadvantaged, "Disadvantaged", "Non-disadvantaged"),
    Median_rate = ifelse(Category == disadvantaged, medr_ad, medr_disad),
...
summary_pop_table <- bind_rows(summary_pop_table, df_refugee) 
```
...
```{r}
df_immigrant <- census_accessibility %>% 
  group_by(!!sym(variable)) %>%
  summarise(
    Total_pop              = sum(CompW1, na.rm = TRUE),
    Respondents            = n(),
...
# Rates
medr_disad <- 
  df_immigrant %>% filter(Category == non_disadvantaged) %>% pull(Median_rounded) / 
  df_immigrant %>% filter(Category == disadvantaged) %>% pull(Median_rounded) 
...
meanr_disad <- 
  df_immigrant %>% filter(Category == non_disadvantaged) %>% pull(Mean_rounded) / 
  df_immigrant %>% filter(Category == disadvantaged) %>% pull(Mean_rounded)
...
summary_pop_table <- bind_rows(summary_pop_table, df_immigrant) 
```
...
# Rates
medr_disad <- 
  df_visbi %>% filter(Category == non_disadvantaged) %>% pull(Median_rounded) / 
  df_visbi %>% filter(Category == disadvantaged) %>% pull(Median_rounded) 
...
meanr_disad <- 
  df_visbi %>% filter(Category == non_disadvantaged) %>% pull(Mean_rounded) / 
  df_visbi %>% filter(Category == disadvantaged) %>% pull(Mean_rounded)
...
summary_pop_table <- bind_rows(summary_pop_table, df_visbi) 
```
...
summarise(
  ...
  medr_disad <- df %>% filter(Equity == "Non-disadvantaged") %>% pull(Median_rounded) / 
                 df %>% filter(Equity == "Disadvantaged") %>% pull(Median_rounded) 

  medr_ad <- 1 / medr_disad

  meanr_disad <- df %>% filter(Equity == "Non-disadvantaged") %>% pull(Mean_rounded) / 
                  df %>% filter(Equity == "Disadvantaged") %>% pull(Mean_rounded)
  ...
  df <- df %>%
	...
	dplyr::select(Variable, Description, Category, Equity, 
	              Total_pop, Respondents, NonZeroCases, NonZeroCases_weighted, 
	              Median, Mean, Mean_rounded, Median_rounded, Median_rate, Mean_rate)
...
-   Statistics must be based on at least 5 respondents, whose sum of weights is at least 10, to be releasable.
-   Thresholds have been established for the minimum number of (unweighted) records to produce quantiles. These are: 8 for medians, 16 for quartiles, 40 for quintiles and deciles, and 400 for percentiles.
-   Statistics are subject to a simplistic percent rounding by deterministically rounding to the nearest multiple of a rounding base. 
...
summary_pop_table_support <- summary_pop_table %>%
  ...
  left_join(census_pcd_vetting, by = "PCD") %>% 
    filter(Population_PCD >= 40, Respondents_PCD >= 5, Respondents >= 5, Total_pop >= 10) %>%
  mutate(Total_pop_rounded = round(Total_pop / 5) * 5) %>% 
  dplyr::select(Variable, Description, Category, Equity, 
                Total_pop, Total_pop_rounded, Respondents, 
                NonZeroCases, NonZeroCases_weighted,
                Median, Mean, Median_rounded, Mean_rounded, 
                Median_rate, Mean_rate, PCD, 
                Respondents_PCD, Population_PCD, Respondents_PCD)
...
aux_pop_table <- summary_pop_table_support %>% 
  ...
  dplyr::summarize(Total_pop = sum(Total_pop),
...
pvalue_table_support <- pvalue_table %>% 
  ...
  mutate(PCD = code_pcd) %>% 
  left_join(census_pcd_vetting, by = "PCD") %>% 
  dplyr::select(Variable, Description, Disadvantaged, Total_pop, Respondents, P_value, Significance, PCD, Population_PCD, Respondents_PCD) %>% 
  filter(Population_PCD >= 40, Respondents_PCD >= 5, Respondents >= 5, Total_pop >= 10)
```
...
```{r directory-export}
diretorio_export_figures <- paste0(here(),"/data-raw/output/PCD",code_pcd,"/DA/disadvantaged-figures/") # Update the address to export the data, if necessary

if(!dir.exists(diretorio_export_figures)){
  dir.create(diretorio_export_figures, recursive = TRUE)}

diretorio_export_tables <- paste0(here(),"/data-raw/output/PCD",code_pcd,"/DA/disadvantaged-tables/") # Update the address to export the data, if necessary
...
census_accessibility_long <- census_accessibility %>%
  ...
  left_join(summary_pop_table_support, by = c("Variable" = "Variable", 
                                      "Category" = "Category"))

summary_data_boxplot <- census_accessibility_long %>% 
...
description_order <- summary_pop_table_support %>% 
  filter(Variable != "General_Pop_label") %>%
  group_by(Description) %>%
  summarise(median_overall = max(Median_rounded[Equity == "Non-disadvantaged"], na.rm = TRUE)) %>%
  arrange(desc(median_overall)) %>%
  pull(Description)

description_order

summary_pop_table_ordered <- summary_pop_table_support %>% 
  filter(Variable != "General_Pop_label") %>%
  mutate(
    Description = factor(Description, levels = description_order),
    Equity = factor(Equity, levels = c("Non-disadvantaged", "Disadvantaged"))
...
ggplot(aes(y = Description, x = Median_rounded, fill = Equity)) +
...
geom_text(aes(label = Category, x = 3),
          ...
          hjust = 0,  
          size = 2.5) + 
geom_text(aes(label = Median_rounded),
          position = position_dodge(width = 0.9),
          hjust = -0.1,  
          size = 2.5) +
  labs(
...
description_order_rate <- summary_pop_table_support %>% 
  filter(Variable != "General_Pop_label") %>%
  group_by(Description) %>%
...
summary_pop_table_ordered_rate <- summary_pop_table_support %>% 
  filter(Variable != "General_Pop_label") %>%
  mutate(
...
description_order_racial <- summary_dvismin_table_support %>%
  group_by(Description) %>%
  summarise(median_overall = max(Median_rounded[Equity == "Non-disadvantaged"], na.rm = TRUE)) %>%
  arrange(desc(median_overall)) %>%
  pull(Description)

summary_racial_table_ordered <- summary_dvismin_table_support %>%
  mutate(
...
```{r median-vismin-pop}
description_order_rate_racial <- summary_dvismin_table_support %>%
  group_by(Description) %>%
  summarise(median_overall = max(Median_rate[Equity == "Non-disadvantaged"], na.rm = TRUE)) %>%
  arrange(desc(median_overall)) %>%
  pull(Description)

summary_pop_table_ordered_rate_racial <- summary_dvismin_table_support %>%
  mutate(
...
description_order <- summary_pop_table_support %>% 
  filter(Variable != "General_Pop_label") %>%
  group_by(Description) %>%
  summarise(mean_overall = max(Mean_rounded[Equity == "Non-disadvantaged"], na.rm = TRUE)) %>%
...
ggplot(aes(y = Description, x = Mean_rounded, fill = Equity)) +
geom_bar(stat="identity", position=position_dodge()) +
scale_fill_brewer(palette="Reds") + 
geom_text(aes(label = Category, x = 3),
          position = position_dodge(width = 0.9),
          hjust = 0,  
          size = 2.5) + 
geom_text(aes(label = Mean_rounded),
          position = position_dodge(width = 0.9),
          hjust = -0.1,  
          size = 2.5) +
  labs(
...
# Release - all groups
write.csv(summary_pop_table_support[,c("Variable","Description","Category","Equity","Total_pop_rounded", "Median_rounded", "Mean_rounded", "Median_rate", "Mean_rate")], paste0(diretorio_export_tables, "summary_pop_table_release.csv"), row.names=FALSE)
...
write.csv(summary_dvismin_table_support[,c("Variable","Description","Category","Equity","Total_pop_rounded", "Median_rounded", "Mean_rounded", "Median_rate", "Mean_rate")], paste0(diretorio_export_tables, "summary_vmin_table_release.csv"), row.names=FALSE)
...
```{r}
# Release
summary_pop_excel <- createWorkbook()
addWorksheet(summary_pop_excel, "general")
addWorksheet(summary_pop_excel, "visible_minorities")

writeData(summary_pop_excel, "general", summary_pop_table_support[,c("Description","Category","Equity","Total_pop_rounded", "Median_rounded", "Mean_rounded", "Median_rate", "Mean_rate")])

writeData(summary_pop_excel, "visible_minorities", summary_dvismin_table_support[,c("Description","Category","Equity","Total_pop_rounded", "Median_rounded", "Mean_rounded", "Median_rate", "Mean_rate")])

## Commenting and highlighting the general sheet

writeComment(summary_pop_excel, "general", col = 1, row = 1, comment = createComment("Type of sociodemographic information to be analyzed.", author = "Author"))
writeComment(summary_pop_excel, "general", col = 2, row = 1, comment = createComment("Category of the sociodomegraphic information to be analyzed.", author = "Author"))
writeComment(summary_pop_excel, "general", col = 3, row = 1, comment = createComment("Wheter this group is considered as disadvantaged in terms of the transportation literature.", author = "Author"))
writeComment(summary_pop_excel, "general", col = 4, row = 1, comment = createComment("Total population (weighted sum, rounded) in this group.", author = "Author"))
writeComment(summary_pop_excel, "general", col = 5, row = 1, comment = createComment("Median (rounded) of job accessibility of this population group. The rounding was realized in accordance to the weighted sum of non-zero cases", author = "Author"))
writeComment(summary_pop_excel, "general", col = 6, row = 1, comment = createComment("Mean (rounded) of job accessibility of this population group. The rounding was realized in accordance to the weighted sum of non-zero cases.", author = "Author"))
writeComment(summary_pop_excel, "general", col = 7, row = 1, comment = createComment("Rate of rounded medians between population from the same descriptor.", author = "Author"))
writeComment(summary_pop_excel, "general", col = 8, row = 1, comment = createComment("Rate of rounded means between population from the same descriptor.", author = "Author"))

addStyle(summary_pop_excel, sheet = c("general"),
         style = createStyle(fgFill = "yellow"),
         rows = 1:(nrow(summary_pop_table_support)+1),
         cols = c(1:8),
         gridExpand = TRUE)

writeComment(summary_pop_excel, "visible_minorities", col = 1, row = 1, comment = createComment("Type of sociodemographic information to be analyzed.", author = "Author"))
writeComment(summary_pop_excel, "visible_minorities", col = 2, row = 1, comment = createComment("Category of the sociodomegraphic information to be analyzed.", author = "Author"))
writeComment(summary_pop_excel, "visible_minorities", col = 3, row = 1, comment = createComment("Wheter this group is considered as disadvantaged in terms of the transportation literature.", author = "Author"))
writeComment(summary_pop_excel, "visible_minorities", col = 4, row = 1, comment = createComment("Total population (weighted sum, rounded) in this group.", author = "Author"))
writeComment(summary_pop_excel, "visible_minorities", col = 5, row = 1, comment = createComment("Median (rounded) of job accessibility of this population group. The rounding was realized in accordance to the weighted sum of non-zero cases", author = "Author"))
writeComment(summary_pop_excel, "visible_minorities", col = 6, row = 1, comment = createComment("Mean (rounded) of job accessibility of this population group. The rounding was realized in accordance to the weighted sum of non-zero cases.", author = "Author"))
writeComment(summary_pop_excel, "visible_minorities", col = 7, row = 1, comment = createComment("Rate of rounded medians between population from the same descriptor.", author = "Author"))
writeComment(summary_pop_excel, "visible_minorities", col = 8, row = 1, comment = createComment("Rate of rounded means between population from the same descriptor.", author = "Author"))

addStyle(summary_pop_excel, sheet = c("visible_minorities"),
         style = createStyle(fgFill = "yellow"),
         rows = 1:(nrow(summary_dvismin_table_support)+1),
         cols = c(1:8),
         gridExpand = TRUE)


saveWorkbook(summary_pop_excel, paste0(diretorio_export_tables, "summary_pop_release.xlsx"), overwrite = TRUE)

#SUPPORT 

summary_pop_excel_support <- createWorkbook()
addWorksheet(summary_pop_excel_support, "general")
addWorksheet(summary_pop_excel_support, "visible_minorities")

writeData(summary_pop_excel_support, "general", summary_pop_table_support)

writeData(summary_pop_excel_support, "visible_minorities", summary_dvismin_table_support)


writeComment(summary_pop_excel_support, "general", col = 1, row = 1, comment = createComment("Census variable used to obtain the information.", author = "Author"))
writeComment(summary_pop_excel_support, "general", col = 2, row = 1, comment = createComment("Type of sociodemographic information to be analyzed.", author = "Author"))
writeComment(summary_pop_excel_support, "general", col = 3, row = 1, comment = createComment("Category of the sociodomegraphic information to be analyzed.", author = "Author"))
writeComment(summary_pop_excel_support, "general", col = 4, row = 1, comment = createComment("Wheter this group is considered as disadvantaged in terms of the transportation literature.", author = "Author"))
writeComment(summary_pop_excel_support, "general", col = 5, row = 1, comment = createComment("Total population (weighted sum, not rounded) in this group.", author = "Author"))
writeComment(summary_pop_excel_support, "general", col = 6, row = 1, comment = createComment("Total population (weighted sum, rounded) in this group.", author = "Author"))
writeComment(summary_pop_excel_support, "general", col = 7, row = 1, comment = createComment("Count of respondents in this group.", author = "Author"))
writeComment(summary_pop_excel_support, "general", col = 8, row = 1, comment = createComment("Count of respondents with non-zero cases in job accessibility value. ", author = "Author"))
writeComment(summary_pop_excel_support, "general", col = 9, row = 1, comment = createComment("Weighted sum of respondents with non-zero cases in job accessibility value.", author = "Author"))
writeComment(summary_pop_excel_support, "general", col = 10, row = 1, comment = createComment("Median (not rounded) of job accessibility of this population group.", author = "Author"))
writeComment(summary_pop_excel_support, "general", col = 11, row = 1, comment = createComment("Mean (not rounded) of job accessibility of this population group.", author = "Author"))
writeComment(summary_pop_excel_support, "general", col = 12, row = 1, comment = createComment("Median (rounded) of job accessibility of this population group. The rounding was realized in accordance to the weighted sum of non-zero cases.", author = "Author"))
writeComment(summary_pop_excel_support, "general", col = 13, row = 1, comment = createComment("Mean (rounded) of job accessibility of this population group. The rounding was realized in accordance to the weighted sum of non-zero cases.", author = "Author"))
writeComment(summary_pop_excel_support, "general", col = 14, row = 1, comment = createComment("Rate of rounded medians between population from the same descriptor.", author = "Author"))
writeComment(summary_pop_excel_support, "general", col = 15, row = 1, comment = createComment("Rate of rounded means between population from the same descriptor.", author = "Author"))
writeComment(summary_pop_excel_support, "general", col = 16, row = 1, comment = createComment("Census Division of the population.", author = "Author"))
writeComment(summary_pop_excel_support, "general", col = 17, row = 1, comment = createComment("Count of respondents in this census division.", author = "Author"))
writeComment(summary_pop_excel_support, "general", col = 18, row = 1, comment = createComment("Total population (weighted) in this census division.", author = "Author"))

addStyle(summary_pop_excel_support, sheet = c("general"),
         style = createStyle(fgFill = "yellow"),
         rows = 1:(nrow(summary_pop_table_support)+1),
         cols = c(2,3,4,6,12,13,14,15),
         gridExpand = TRUE)

writeComment(summary_pop_excel_support, "visible_minorities", col = 1, row = 1, comment = createComment("Census variable used to obtain the information.", author = "Author"))
writeComment(summary_pop_excel_support, "visible_minorities", col = 2, row = 1, comment = createComment("Type of sociodemographic information to be analyzed.", author = "Author"))
writeComment(summary_pop_excel_support, "visible_minorities", col = 3, row = 1, comment = createComment("Category of the sociodomegraphic information to be analyzed.", author = "Author"))
writeComment(summary_pop_excel_support, "visible_minorities", col = 4, row = 1, comment = createComment("Wheter this group is considered as disadvantaged in terms of the transportation literature.", author = "Author"))
writeComment(summary_pop_excel_support, "visible_minorities", col = 5, row = 1, comment = createComment("Total population (weighted sum, not rounded) in this group.", author = "Author"))
writeComment(summary_pop_excel_support, "visible_minorities", col = 6, row = 1, comment = createComment("Total population (weighted sum, rounded) in this group.", author = "Author"))
writeComment(summary_pop_excel_support, "visible_minorities", col = 7, row = 1, comment = createComment("Count of respondents in this group.", author = "Author"))
writeComment(summary_pop_excel_support, "visible_minorities", col = 8, row = 1, comment = createComment("Count of respondents with non-zero cases in job accessibility value. ", author = "Author"))
writeComment(summary_pop_excel_support, "visible_minorities", col = 9, row = 1, comment = createComment("Weighted sum of respondents with non-zero cases in job accessibility value.", author = "Author"))
writeComment(summary_pop_excel_support, "visible_minorities", col = 10, row = 1, comment = createComment("Median (not rounded) of job accessibility of this population group.", author = "Author"))
writeComment(summary_pop_excel_support, "visible_minorities", col = 11, row = 1, comment = createComment("Mean (not rounded) of job accessibility of this population group.", author = "Author"))
writeComment(summary_pop_excel_support, "visible_minorities", col = 12, row = 1, comment = createComment("Median (rounded) of job accessibility of this population group. The rounding was realized in accordance to the weighted sum of non-zero cases.", author = "Author"))
writeComment(summary_pop_excel_support, "visible_minorities", col = 13, row = 1, comment = createComment("Mean (rounded) of job accessibility of this population group. The rounding was realized in accordance to the weighted sum of non-zero cases.", author = "Author"))
writeComment(summary_pop_excel_support, "visible_minorities", col = 14, row = 1, comment = createComment("Rate of rounded medians between population from the same descriptor.", author = "Author"))
writeComment(summary_pop_excel_support, "visible_minorities", col = 15, row = 1, comment = createComment("Rate of rounded means between population from the same descriptor.", author = "Author"))
writeComment(summary_pop_excel_support, "visible_minorities", col = 16, row = 1, comment = createComment("Census Division of the population.", author = "Author"))
writeComment(summary_pop_excel_support, "visible_minorities", col = 17, row = 1, comment = createComment("Count of respondents in this census division.", author = "Author"))
writeComment(summary_pop_excel_support, "visible_minorities", col = 18, row = 1, comment = createComment("Total population (weighted) in this census division.", author = "Author"))

addStyle(summary_pop_excel_support, sheet = c("visible_minorities"),
         style = createStyle(fgFill = "yellow"),
         rows = 1:(nrow(summary_dvismin_table_support)+1),
         cols = c(2,3,4,6,12,13,14,15),
         gridExpand = TRUE)

saveWorkbook(summary_pop_excel_support, paste0(diretorio_export_tables, "summary_pop_support.xlsx"), overwrite = TRUE)
```
...
Export of box plot table: 

```{r export-summary-tab}
write.csv(summary_data_ordered, paste0(diretorio_export_tables, "boxplot_summary_table.csv"), row.names=FALSE)
```
