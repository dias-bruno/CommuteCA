...
```{r}
directory_spatial_files <- paste0(here(), "/data-raw/output/PCD3525/spatial-files/") # Update this address if necessary
...
mode_metrics_ct <- 
  ...
  distinct(CTUID, PwMode, .keep_all = TRUE)
...
```{r directory-export}
diretorio_export_figures <- paste0(here(),"/data-raw/output/",code,"/CT/accessibility-figures/") # Update the address to export the data, if necessary
...
modal_SA_percents <- mode_metrics_ct %>% 
  group_by(PwMode) %>%
  summarize(total_labour_fource = sum(labour_force, na.rm = TRUE),
            SA_meam = mean(SA_im),
