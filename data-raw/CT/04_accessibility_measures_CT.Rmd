...
```{r read-ttm}
ttm_file <- paste0(here(),"/data-raw/output/PCD",code,"/CT/travel_times/ttm.csv")
...
```{r directory-export}
diretorio_export <- paste0(here(),"/data-raw/output/PCD",code,"/CT/accessibility-measures/") # Update the address to export the data, if necessary
...
land_use_general <- land_use_mode %>%
  group_by(CTUID) %>%
  summarize(labour_force = sum(labour_force, na.rm = TRUE),
            across(.cols = -labour_force, .fns  = first ),
            .groups = "drop") 
