...
```{r}
census_city %>% 
  dplyr::summarise(Median = Hmisc::wtd.quantile(CfInc_AT, weights = CompW1, probs = 0.5))
```
...
```{r}
census_city %>% 
  dplyr::summarise(Second_Q = Hmisc::wtd.quantile(CfInc_AT, weights = CompW1, probs = 0.25),
                   Median = Hmisc::wtd.quantile(CfInc_AT, weights = CompW1, probs = 0.5), 
                   Third_Q = Hmisc::wtd.quantile(CfInc_AT, weights = CompW1, probs = 0.75))
```
...
CfInc_AT_statistics <- census_city %>% 
  dplyr::summarise(Mean = Hmisc::wtd.mean(CfInc_AT, weights = CompW1),
                   Median = Hmisc::wtd.quantile(CfInc_AT, weights = CompW1, probs = 0.5),
                   SD = sqrt(Hmisc::wtd.var(CfInc_AT, weights = CompW1)), 
                   Max = max(CfInc_AT),
                   Min = min(CfInc_AT))
```
