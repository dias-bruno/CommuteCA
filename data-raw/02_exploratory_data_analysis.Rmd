...
dataset used in this demonstration is synthetic data produced to
replicate the variables available in the original Census of Population.
The synthetic data contains 200,000 rows and 17 columns. As in the
...
If you want to work with the original Census dataset, the process for performing exploratory data analysis will be the same as for the synthetic data, except that you will have to update the address of the file in the chunk[^3] called *load-census-data*.
...
We will use the function `read.csv()`to read the sheets in the format
...
will use the synthetic version of the survey. If you want to use the
...
this R markdown in another project or if the synthetic data is stored in
a different folder that does not follow the pattern of the `COMMUCECA21`
package.

```{r census-file-address}
census_address <- paste0(here::here(), "/data-raw/input/synthetic/census_synthetic_v3.csv") 
```
...
force, or a person who works from home and therefore does not need to
commute).
...
`LBR_FORC` variable (considering the synthetic data), representing
...
of the deviations from the mean: $$ \sigma^2 = \frac{1}{n}\sum_{i=1}^nz_i $$
