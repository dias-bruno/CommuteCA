...
```{r}
#load(paste0(here::here(),"/data/dissemination_areas_ca21.rmd"))
...
vetting_ct_mode <- census %>%
  left_join(da_cts, by = c("PRCDDA" = "DAUID")) %>% 
  filter(!is.na(CTUID)) %>%
  group_by(CTUID, PwMode) %>%
  summarize(Respondents_mode = n()) %>%
  filter(!is.na(PwMode)) %>% 
  dplyr::select(CTUID, PwMode, Respondents_mode)
...
land_use_mode_CT_support <- land_use_CT %>% 
  left_join(vetting_ct_mode, by = c('CTUID' = 'CTUID','PwMode' = 'PwMode')) %>%
  left_join(vetting_ct, by = c('CTUID' = 'CTUID')) %>% 
  dplyr::select(CTUID, PwMode, labour_force, jobs, PRUID, PCD, CMAUID, CMANAME, Respondents_mode, Population_CT)
...
land_use_general_CT_support <- land_use_general_CT %>%
  left_join(vetting_ct, by = c('CTUID' = 'CTUID')) %>% 
  dplyr::select(CTUID, labour_force, jobs, PRUID, PCD, CMAUID, CMANAME, Population_CT, Respondents_CT)
```
...
land_use_general_CT_support <- land_use_general_CT_support %>% 
  filter(labour_force > 10, Population_CT >= 40, jobs >= 40, Respondents_CT >= 5) %>%
...
land_use_mode_CT_support <- land_use_mode_CT_support %>% 
  filter(labour_force > 10, Population_CT >= 40, jobs >= 40, Respondents_mode >= 5) %>%
  mutate(jobs_rounded = round(jobs / 5) * 5,
         labour_force_rounded = round(labour_force / 5) * 5)
```
...
```{r directory-export}
diretorio_export <- paste0(here::here(),"/data-raw/output/CT/") #Update the address to export the data, if necessary
...
write.csv(land_use_general_CT_support[,c("CTUID","PCD","CMAUID","CMANAME","PRUID","labour_force","labour_force_rounded","jobs","jobs_rounded", "Population_CT", "Respondents_CT")],
...
write.csv(land_use_mode_CT_support[,c("CTUID","PCD","CMAUID","CMANAME","PRUID","PwMode","labour_force","labour_force_rounded","jobs","jobs_rounded", "Population_CT", "Respondents_mode")], paste0(diretorio_export, "land_use_mode_CT_support.csv"), row.names=FALSE)
```
