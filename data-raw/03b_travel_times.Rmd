...
# For census tract
# directory <- paste0(here(), "/data-raw/aux-files/" , code, "/CT/") # For census tract
# spatial_unit <- st_read(paste0(directory, "census_tracts.shp"))

# For dissemination areas
directory <- paste0(here(), "/data-raw/aux-files/" , code, "/DA/") # For dissemination areas
...
```{r r5r-core}
r5r_core <- setup_r5(data_path = paste0(directory, "travel_times"), verbose = FALSE)
```
...
```{r export-ttm}
write.csv(ttm_spatial_unit[,c("from_id","to_id","travel_time","PwMode","PCD","CMAUID","PRUID")], paste0(directory, "travel_times/ttm.csv"), row.names=FALSE)
```
