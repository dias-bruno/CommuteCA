---
title: "Calculating the accessibility and spatial availability of jobs"
...
The dataset used in this demonstration is synthetic data produced to
replicate the variables available in the original Census of Population.
The synthetic data contains 200,000 rows and 17 columns. As in the original census data,
each row refers to a respondent and each column refers to a
variable[^2].\
The creation of synthetic data was necessary because the surveys
...
synthetic data, except that you will have to update the address of the
...
[^5]: Although we have provided synthetic census data for all Canadian
...
[^9]: For this demonstration, we will use the case of the city of
    ...
    from the Census in an RDC office or the synthetic data for all
    locations in Canada, *update* the address in the chunk.
    
```{r}
files_address <- paste0(here(),"/data-raw/input/synthetic/census_synthetic_v3.csv") # Census address
```
...
test_distributions <- function(x){
  gamma_ <<- fitdistrplus::fitdist(data=x, "gamma", method="mme")
  lnorm_ <<- fitdistrplus::fitdist(data=x, "lnorm", method="mme")
  norm_ <<- fitdistrplus::fitdist(data=x, "norm", method="mme")
  exp_ <<- fitdistrplus::fitdist(data=x, "exp", method="mme")
  unif_ <<- fitdistrplus::fitdist(data=x, "unif", method="mme")
  
  chosen_function <- lowest_aic(c(lnorm_$aic, gamma_$aic, unif_$aic, norm_$aic, exp_$aic))
...
```{r bike-function}
mode <- 'Bike'
bike_function <- test_distributions(tld_empirical[tld_empirical$mode == mode, ]$PWDUR)
bike_function
```
...
```{r car-function}
mode <- 'Car/motor'
car_function <- test_distributions(tld_empirical[tld_empirical$mode == mode, ]$PWDUR)
car_function
...
```{r transit-function}
mode <- 'Transit'
transit_function <- test_distributions(tld_empirical[tld_empirical$mode == mode, ]$PWDUR)
transit_function
```
...
```{r walk-function}
mode <- 'Walk'
walk_function <- test_distributions(tld_empirical[tld_empirical$mode == mode, ]$PWDUR)
walk_function
```
...
Reading the travel time table created in the *01_travel_times.Rmd*^[*If you are working with another study area or with the original Census data, don't forget to change the address file for the next chunk.*]:

```{r read-ttm}
ttm <- read.csv("travel_times\\PCD3520\\ttm.csv") # Travel time table address
```
...
ttm <- ttm %>% 
  left_join(weights, by= c("PRCDDA","PWDA","PwMode"))

ttm[is.na(ttm$weights),]$weights <- 0
...
for(cma in unique(ttm$CMA)){
  ...
  for(mode in (unique(ttm_cma$PwMode))){
    ...
    if(length(ttm_cma_mode$travel_time) == 1){
      ...
      best_mode_function <- test_distributions_weighted(x = ttm_cma_mode$travel_time, 
...
land_use[is.na(land_use$jobs),]$jobs <- 0
```
...
accessibility_table <- ttm_f %>%
  left_join(land_use[,c("PRCDDA","PwMode","labour_force")], by = c("PRCDDA","PwMode")) %>%
  left_join(land_use[,c("PRCDDA","PwMode","jobs")], by = c("PWDA" = "PRCDDA","PwMode"))

accessibility_table[is.na(accessibility_table$labour_force),]$labour_force <- 0
accessibility_table[is.na(accessibility_table$jobs),]$jobs <- 0
```
...
- Counts must be based on at least 5 respondents ($n ≥ 5$), with a sum of weights of at least 10 ($\sum{CompW1} ≥ 10$), to be releasable.
...
vetting_da_mode <- census %>%
  group_by(PRCDDA, PwMode) %>%
  summarize(Respondents_mode = n()) %>% 
         dplyr::select(PRCDDA, PwMode, Respondents_mode)

vetting_da <- census %>%
  group_by(PRCDDA) %>%
  summarize(Population_DA = sum(CompW1),
            Respondents_DA = n()) %>% 
         dplyr::select(PRCDDA, Population_DA, Respondents_DA)
```
...
```{r creating-release}
# Land Use Tables
land_use_mode_release <- land_use %>% 
  left_join(vetting_da_mode, by = c('PRCDDA' = 'PRCDDA','PwMode' = 'PwMode')) %>%
  left_join(vetting_da, by = c('PRCDDA' = 'PRCDDA')) %>% 
  dplyr::select(PRCDDA, PwMode, labour_force, jobs, Respondents_mode, Population_DA)

land_use_general <- land_use %>% 
    group_by(PRCDDA) %>%
    summarize(labour_force = sum(labour_force),
              jobs = first(jobs))

land_use_general_release <- land_use_general %>%
  left_join(vetting_da, by = c('PRCDDA' = 'PRCDDA')) %>% 
  dplyr::select(PRCDDA, labour_force, jobs, Population_DA, Respondents_DA)

# Hansen-type Accessibility
HT_mode_release <- HT_mode %>% 
  left_join(vetting_da_mode, by = c('PRCDDA' = 'PRCDDA','PwMode' = 'PwMode')) %>%
  left_join(vetting_da, by = c('PRCDDA' = 'PRCDDA')) %>% 
  dplyr::select(PRCDDA, PwMode, HT_im, Respondents_mode, Population_DA)

HT_release <- HT %>% 
  left_join(vetting_da, by = c('PRCDDA' = 'PRCDDA')) %>% 
  dplyr::select(PRCDDA, HT_i, Population_DA, Respondents_DA)

# Spatial Availability 
SA_mode_release <- SA_mode %>% 
  left_join(vetting_da_mode, by = c('PRCDDA' = 'PRCDDA','PwMode' = 'PwMode')) %>%
  left_join(vetting_da, by = c('PRCDDA' = 'PRCDDA')) %>% 
  dplyr::select(PRCDDA, PwMode, SA_im, Respondents_mode, Population_DA)

SA_release <- SA %>% 
  left_join(vetting_da, by = c('PRCDDA' = 'PRCDDA')) %>% 
  dplyr::select(PRCDDA, SA_i, Population_DA, Respondents_DA)
```

Apply the rules to avoid confidentiality risks: 

```{r filter-release}
land_use_mode_release <- land_use_mode_release[land_use_mode_release$Population_DA >= 40 & !is.na(land_use_mode_release$Respondents_mode) & land_use_mode_release$Respondents_mode >= 5, ]

land_use_general_release <- land_use_general_release[land_use_general_release$Population_DA >= 40 & !is.na(land_use_general_release$Respondents_DA) & land_use_general_release$Respondents_DA >= 5, ]

HT_mode_release <- HT_mode_release[HT_mode_release$Population_DA >= 40 & !is.na(HT_mode_release$Respondents_mode) & HT_mode_release$Respondents_mode >= 5, ]

HT_release <- HT_release[HT_release$Population_DA >= 40 & !is.na(HT_release$Respondents_DA) & HT_release$Respondents_DA >= 5, ]

SA_mode_release <- SA_mode_release[SA_mode_release$Population_DA >= 40 & !is.na(SA_mode_release$Respondents_mode) & SA_mode_release$Respondents_mode >= 5, ]

SA_release <- SA_release[SA_release$Population_DA >= 40 & !is.na(SA_release$Respondents_DA) & SA_release$Respondents_DA >= 5, ]
```
...
In our case, we filtered the census data according to the variable *PCD
= 3520* (Toronto census division). For this reason, we will assign
...
```{r directory-export}
diretorio_export <- paste0(here::here(),"/data-raw/output/", type, code, "/") #Update the address to export the data, if necessary
...
```{r export-land-use}
# land use table without distinction by mode
write.csv(land_use_general_release[,c("PRCDDA","labour_force","jobs")], paste0(diretorio_export, "land_use_general.csv"), row.names=FALSE)

write.csv(land_use_general_release, paste0(diretorio_export, "land_use_general_release.csv"), row.names=FALSE)

# land use with distinction by mode
write.csv(land_use_mode_release[,c("PRCDDA","PwMode","labour_force","jobs")], paste0(diretorio_export, "land_use_mode.csv"), row.names=FALSE)

write.csv(land_use_mode_release[,c("PRCDDA","PwMode","labour_force","jobs")], paste0(diretorio_export, "land_use_mode_release.csv"), row.names=FALSE)
...
```{r export-HT_mode}
write.csv(HT_mode_release[,c("PRCDDA","PwMode","HT_im")], paste0(diretorio_export, "HT_mode.csv"), row.names=FALSE)
write.csv(HT_mode_release, paste0(diretorio_export, "HT_mode_release.csv"), row.names=FALSE)
```

```{r export-HT}
write.csv(HT_release[,c("PRCDDA","HT_i")], paste0(diretorio_export, "HT.csv"), row.names=FALSE)
write.csv(HT_release, paste0(diretorio_export, "HT_release.csv"), row.names=FALSE)
```

Export of spatial availability tables:

```{r export-SA}
write.csv(SA_release[,c("PRCDDA","SA_i")], paste0(diretorio_export, "SA.csv"), row.names=FALSE)
write.csv(SA_release, paste0(diretorio_export, "SA_release.csv"), row.names=FALSE)
```

```{r export-SA_mode}
write.csv(SA_mode_release[,c("PRCDDA","PwMode","SA_im")], paste0(diretorio_export, "SA_mode.csv"), row.names=FALSE)
write.csv(SA_mode_release, paste0(diretorio_export, "SA_mode_release.csv"), row.names=FALSE)
```
