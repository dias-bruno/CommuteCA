...
```{r creating-release}
# Land Use Tables
land_use_mode_support <- land_use %>% 
...
land_use_general_support <- land_use_general %>%
  left_join(vetting_da, by = c('PRCDDA' = 'PRCDDA')) %>% 
...
```{r}
land_use_general_support <- land_use_general_support %>% 
  filter(labour_force > 10, Population_DA >= 40, jobs >= 40, Respondents_DA >= 5) %>%
  mutate(jobs_rounded = round(jobs / 5) * 5,
         labour_force_rounded = round(labour_force / 5) * 5)


land_use_mode_support <- land_use_mode_support %>% 
  filter(labour_force > 10, Population_DA >= 40, jobs >= 40, Respondents_mode >= 5) %>%
  mutate(jobs_rounded = round(jobs / 5) * 5,
         labour_force_rounded = round(labour_force / 5) * 5)
```
...
```{r filter-support}
HT_mode_support <- HT_mode_support[HT_mode_support$Population_DA >= 40 & !is.na(HT_mode_support$Respondents_mode) & HT_mode_support$Respondents_mode >= 5, ]

HT_support <- HT_support[HT_support$Population_DA >= 40 & !is.na(HT_support$Respondents_DA) & HT_support$Respondents_DA >= 5, ]

SA_mode_support <- SA_mode_support[SA_mode_support$Population_DA >= 40 & !is.na(SA_mode_support$Respondents_mode) & SA_mode_support$Respondents_mode >= 5, ]

SA_support <- SA_support[SA_support$Population_DA >= 40 & !is.na(SA_support$Respondents_DA) & SA_support$Respondents_DA >= 5, ]
```
...
# HT tables
write.csv(HT, paste0(diretorio_export, "HT_general_original.csv"), row.names=FALSE)
write.csv(HT_mode, paste0(diretorio_export, "HT_mode_original.csv"), row.names=FALSE)

# SA tables
write.csv(SA, paste0(diretorio_export, "SA_general_original.csv"), row.names=FALSE)
write.csv(SA_mode, paste0(diretorio_export, "SA_mode_original.csv"), row.names=FALSE)
```

Export of land use tables:

```{r export-land-use-release}
write.csv(land_use_general_support[,c("PRCDDA","labour_force_rounded","jobs_rounded")], paste0(diretorio_export, "land_use_general_DA_release.csv"), row.names=FALSE)

write.csv(land_use_general_support, paste0(diretorio_export, "land_use_general_DA_support.csv"), row.names=FALSE)

write.csv(land_use_mode_support[,c("PRCDDA","PwMode","labour_force_rounded","jobs_rounded")], paste0(diretorio_export, "land_use_mode_DA_release.csv"), row.names=FALSE)

write.csv(land_use_mode_support, paste0(diretorio_export, "land_use_mode_DA_support.csv"), row.names=FALSE)
```
